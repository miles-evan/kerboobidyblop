...
export default class Board extends GameObject {
	...
	static getPositionOfTile(lane: number, rank: number): [number, number] {
		// lane 0 is left most col, rank 0 is bottom most row
		return [4 * (8 + 16*lane), 4 * (10 + 16*(9-rank))];
	}
	
	step() {
		if(Game.isKeyPressed("ArrowDown")) {
			new Spell(...Board.getPositionOfTile(1, 0), 2, 1);
		}
		if(Game.isKeyPressed("ArrowLeft")) {
			new Spell(...Board.getPositionOfTile(0, 0), 2, 1);
		}
		if(Game.isKeyPressed("ArrowRight")) {
			new Spell(...Board.getPositionOfTile(2, 0), 2, 1);
		}
	}
